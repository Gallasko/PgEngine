...
namespace pg
{
    ...
    struct INetworkBackend
    {
        ...
        inline virtual void writeU16BE(uint8_t* buf, uint16_t v) = 0;
        inline virtual void writeU32BE(uint8_t* buf, uint32_t v) = 0;
        inline virtual void writeU64BE(uint8_t* buf, uint64_t v) = 0;

        inline virtual uint16_t readU16BE(const uint8_t* buf) = 0;
        inline virtual uint32_t readU32BE(const uint8_t* buf) = 0;
        inline virtual uint64_t readU64BE(const uint8_t* buf) = 0;
