...
:   m_seed  (seed)
{
    m_noiseParameters.octaves       = 3;
    m_noiseParameters.amplitude     = 5;
    m_noiseParameters.smoothness    = 50;
    m_noiseParameters.heightOffset  = -1;
    m_noiseParameters.roughness     = 0.70;
}

double NoiseGenerator::noise2D(int x, int z) const noexcept
{
        auto newX = x;
        auto newZ = z;
        ...
        for (auto a = 0; a < m_noiseParameters.octaves - 1; a++)      //This loops through the octaves.
        {
            ...
            totalValue += sNoise.noise(((double)newX) * frequency / m_noiseParameters.smoothness,
...
double NoiseGenerator::noise3D(int x, int y, int z, double offset) const noexcept
{
    auto newX = x * offset;
    auto newY = y * offset;
    auto newZ = z * offset;

    return sNoise.noise(newX, newY, newZ);
}
