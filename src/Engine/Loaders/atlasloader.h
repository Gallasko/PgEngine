...
namespace pg
{
    ...
    public:
        LoadedAtlas() {}
        LoadedAtlas(const std::string& atlasFile, std::unordered_map<std::string, ParserCallback> callbacks = std::unordered_map<std::string, ParserCallback>())
        {
        ...
        LoadedAtlas(const LoadedAtlas& other) : version(other.version), imagePath(other.imagePath), atlasWidth(other.atlasWidth), atlasHeight(other.atlasHeight), nbTextureId(other.nbTextureId),
            textureList(other.textureList), textureDict(other.textureDict), emptyTexture(other.emptyTexture)
        {
            LOG_ERROR("Atlas", "Copying a loaded atlas should never happen");
        }

        void setFile(const std::string& atlasFile, std::unordered_map<std::string, ParserCallback> callbacks = std::unordered_map<std::string, ParserCallback>());
