...
namespace pg
{
    ...
    struct BackgroundScrollerSystem : public System<InitSys, QueuedListener<TickEvent>, QueuedListener<ResizeEvent>>
    {
        std::string getSystemName() const override { return "Bg scroller System"; }

        // Screen and tile parameters
        float screenWidth = 820.0f;
        float screenHeight = 640.0f;
        float tileSize = 80.0f;              // Size of each tile (square)
